...
function handleGroupFilter(key, value) {
  ...
  requestAnimationFrame(() => {
    ...
    if (key === 'installment') {
      fallbackProducts = state.products.filter(p => isInStock(p, state.soldoutIds, state.revivedIds) && (p.installment_months || 0) === Number(value));
    } else if (key === 'bestFor') {
      fallbackProducts = state.products.filter(p => isInStock(p, state.soldoutIds, state.revivedIds) && (p.best_for_tags || []).includes(String(value)));
    } else if (key === 'game') {
      fallbackProducts = state.products.filter(p => isInStock(p, state.soldoutIds, state.revivedIds) && (p.categories?.games || []).includes(String(value)));
    } else {
      fallbackProducts = state.products.filter(p => isInStock(p, state.soldoutIds, state.revivedIds) && (p.categories?.usage || []).includes(String(value)));
    }
