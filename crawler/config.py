...
# ─── 상품명 -> 게임 태그 키워드 매핑 ────────────────────────
GAME_KEYWORD_MAP = {
    # 게임명 고유 키워드만 허용 — "고사양", "스팀" 같은 일반 단어 제거
    # (이런 단어가 있으면 모든 게이밍 PC에 해당 게임 태그가 붙어버림)
    "리그오브레전드": ["리그오브레전드", "LOL"],  # "롤" 제거 (롤케이지, 롤러 등 오탐)
    "배틀그라운드": ["배틀그라운드", "PUBG"],      # "배그" 유지
    "로스트아크": ["로스트아크"],                  # "로아" 제거 (오탐 가능성)
    "스팀 AAA급 게임": ["사이버펑크", "와일즈", "몬스터헌터", "엘든링", "GTA"],  # "AAA", "고사양", "스팀" 제거
    "발로란트": ["발로란트"],                      # "발로" 제거 (발로나 등 오탐)
    "오버워치2": ["오버워치", "오버워치2"],
    "아이온2": ["아이온2", "아이온 2"],
}
...
# ─── 케이스 색상 감지 키워드 ─────────────────────────────────
# 단일 문자("W", "B")는 오탐이 많아 제외. 최소 2자 이상 명시적 키워드만 사용
CASE_WHITE_KEYWORDS = ["화이트", "WHITE", "White", "WH", "WHT"]
CASE_BLACK_KEYWORDS = ["블랙", "BLACK", "Black", "BLK", "BK"]
