...
categories.games (게임 필터):
  - 타이틀에 게임명이 직접 언급된 제품만 해당 게임 태깅
    - 스팀 AAA급 게임: 타이틀 언급 OR 퍼포먼스/하이엔드 GPU + 게이밍

    best_for_tags (용도 필터):
      - 화이트 감성:  case_color == 화이트
        - 4K 게이밍:   타이틀에 "4K" OR (하이엔드 GPU + 게이밍 primary)
          - QHD 게이밍:  타이틀에 "QHD" OR (퍼포먼스 GPU + 게이밍 primary)
            - AI 공부용:   타이틀에 AI 키워드 OR (AI usage primary + 가격 350만 이하)
              - 로컬 LLM 입문: 타이틀에 LLM 키워드 OR (AI usage 포함 + VRAM 16GB 이상)
              """
...
GPU_KEY_NORMALIZE = {
      "RTX 5070 TI": "RTX 5070 Ti", "RTX 5060 TI": "RTX 5060 Ti",
      "RTX 4080 SUPER": "RTX 4080 Super", "RTX 4070 TI SUPER": "RTX 4070 Ti Super",
      "RTX 4070 TI": "RTX 4070 Ti Super", "RTX 4070 SUPER": "RTX 4070 Super",
      "RTX 4060 TI": "RTX 4060 Ti", "RTX 3060 TI": "RTX 3060 Ti",
      "RTX5060": "RTX 5060", "RTX5070": "RTX 5070",
      "RTX 3090 TI": "RTX 3090 Ti", "RTX 2060 SUPER": "RTX 2060 Super",
      "RX 9070XT": "RX 9070 XT", "RX9070XT": "RX 9070 XT",
...
AAA_TITLE_KEYWORDS = [
      "붉은사막", "사이버펑크", "cyberpunk", "엘든링", "elden ring",
      "위쳐", "witcher", "어쌔신", "assassin", "갓오브워", "god of war",
      "스파이더맨", "spider-man", "fc25", "fc 25", "ea fc",
      "스팀 aaa", "aaa 게임",
]

AI_STUDY_TITLE_KEYWORDS = [
      "AI 학습", "AI학습", "AI 공부", "AI공부", "AI 입문", "AI입문",
      "딥러닝", "머신러닝", "AI 이미지", "AI이미지", "AI 작업용", "AI작업용",
      "nvidia ai", "엔비디아 ai", "AI YJ", "ai yj", "AI 생성", "AI생성",
]

LLM_TITLE_KEYWORDS = ["LLM", "llm", "로컬 LLM", "로컬LLM", "로컬 AI", "로컬AI", "로컬 ai"]

AI_STUDY_MAX_PRICE = 3_500_000
LOCAL_LLM_MIN_VRAM = 16
...
def classify_best_for_tags(product: dict) -> list:
    ...
    PRIORITY = {"영상편집": 1, "3D 모델링": 2, "방송/스트리밍": 3, "AI/딥러닝": 4, "사무/디자인": 5, "게이밍": 6}
    ...
    if case_col in ("화이트", "White", "WHITE"):
              tags.append("화이트 감성")
    is_4k = title_has(name, ["4K", "4k", "4케이"]) or (is_primary_gaming and tier == "하이엔드(4K)")
    if is_4k:
              tags.append("4K 게이밍")
    if "4K 게이밍" not in tags:
              is_qhd = title_has(name, ["QHD", "qhd", "2K", "wqhd", "WQHD"]) or (is_primary_gaming and tier == "퍼포먼스(QHD)")
        if is_qhd:
                      tags.append("QHD 게이밍")
...
def main():
      print("=" * 60)
    print("  제품 분류 엔진 v4 (타이틀 우선 + GPU 폴백)")
    print("=" * 60)
